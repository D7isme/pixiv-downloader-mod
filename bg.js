const LS = {
  ...
  setItem: (a, d) => chrome.storage.local.set({ [a]: d }),
...
chrome.runtime.onInstalled.addListener(async function () {
  (await LS.getItem("welcome")) ||
    (chrome.tabs.create({ url: "welcome.html" }),
    await LS.setItem("welcome", !0));
});
chrome.runtime.onMessageExternal.addListener(function (a, d, b) {
  "pxvdwnex" === a.mode &&
    ("ja" === chrome.i18n.getUILanguage().split("-")[0]
      ? alert_m(
          "PixivDownloader\u304c\u8907\u6570\u8d77\u52d5\u3057\u3066\u3044\u307e\u3059\u3002\u62e1\u5f35\u6a5f\u80fd\u306e\u7ba1\u7406\u30da\u30fc\u30b8\u304b\u3089\u3001\u4f7f\u7528\u3057\u3066\u3044\u306a\u3044\u53e4\u3044PixivDownloader\u3092\u505c\u6b62\u3057\u3066\u304f\u3060\u3055\u3044\u3002"
        )
      : alert_m(
          "Multiple instances of PixivDownloader are running. Please stop any old PixivDownloaders that are not in use from the Extensions Management page."
        ));
});
async function initialize() {
  async function a(f) {
    return f(chrome.runtime.getManifest());
  }
  (function () {
    for (
      var f = [
          "fnbkeopcpjainobjebddfcnnknmfipid",
          "mpeoelpkgbnhobojpafdpopjkmeodpaj",
          "badokdmekghdolcnjiadnpabneleekcp",
        ],
        q = f.length,
        t = 0;
      t < q;
      ...
      f[t] !== chrome.runtime.id &&
        chrome.runtime.sendMessage(f[t], { mode: "pxvdwnex" }).catch((w) => {});
  })();
  if (await LS.getItem("data"))
    try {
      d = JSON.parse(await LS.getItem("data"));
    } catch (f) {
      alert_m(
        "Pixiv Downloader EX MOD\u306e\u8a2d\u5b9a\u30c7\u30fc\u30bf\u304c\u58ca\u308c\u3066\u3044\u307e\u3059\n\u30aa\u30d7\u30b7\u30e7\u30f3\u30da\u30fc\u30b8\u304b\u3089\u521d\u671f\u5316\u3092\u884c\u3063\u3066\u304f\u3060\u3055\u3044"
      );
      return;
    }
  else var d = {};
  a(async function (f) {
    if ((f = await LS.getItem("version"))) {
      a: {
        f = (await LS.getItem("version")).split(".").map(Number);
        for (
          var q = ["1", "6", "45", "201"].map(Number), t = 0;
          t < f.length;
          ++t
        ) {
          if (q.length == t) {
        ...
        f = f.length != q.length ? -1 : 0;
      }
      f = 1 !== f;
    }
    f &&
      ((d.captionTxt = d.captionTxt
        .replace(/\?illust-second\?/g, "?illust-second-p?")
        .replace(/\?illust-minute\?/g, "?illust-minute-p?")
        .replace(/\?illust-hour\?/g, "?illust-hour-p?")),
      (d.dirname = d.dirname
        .replace(/\?illust-second\?/g, "?illust-second-p?")
        .replace(/\?illust-minute\?/g, "?illust-minute-p?")
        .replace(/\?illust-hour\?/g, "?illust-hour-p?")),
      (d.filename = d.filename
        ...
        .replace(/\?illust-minute\?/g, "?illust-minute-p?")
        .replace(/\?illust-hour\?/g, "?illust-hour-p?")),
      (d.novel_captionTxt = d.novel_captionTxt
        .replace(/\?illust-second\?/g, "?illust-second-p?")
        .replace(/\?illust-minute\?/g, "?illust-minute-p?")
        .replace(/\?illust-hour\?/g, "?illust-hour-p?")),
      (d.novel_captionTxtAfter = d.novel_captionTxtAfter
        .replace(/\?illust-second\?/g, "?illust-second-p?")
        .replace(/\?illust-minute\?/g, "?illust-minute-p?")
        .replace(/\?illust-hour\?/g, "?illust-hour-p?")),
      await LS.setItem("data", JSON.stringify(d)));
  });
  d.server_dl && delete d.server_dl;
  var b = {
    filename: "?member-name? - ?title? (?illust-id?)?[ p]??page-zeroformat?",
    star: "0",
    dl_history: !0,
    dl_alert: !0,
    dl_message: !0,
    ...
    iine_dl: !1,
    one_click_bm_dl: !1,
    illust_zoom_select: "0",
    ugoira_zoom_select: "2",
    manga_zoom_select: "3",
    dl_history_erase: !0,
    dl_shortcut_on: !1,
    dl_shortcut_key: "altKey",
    dl_shortcut_abc: "80",
    dl_select: "0",
    manga_select: "1",
    dl_ugoira: "4",
    dirname: "pixiv/?member-name?",
    full_ugoira: !0,
    file_overwrite: "1",
    avi_ecomode: !1,
    illust_downloaded: !1,
  ...
  b.syncId = getUniqueStr();
  b.free_download_mode = "0";
  b.oneclick_dl = !1;
  b.illust_zoom_bm_win = !0;
  b.erase_emoji = !1;
  b.captionDLZIP = !1;
  b.manga_st_one = !1;
  b.auto_dl_wait = "1000";
  b.def_dl_wait = "150";
  b.alldl_errskip = !0;
  b.select_language = "default";
  var l = !1,
    n;
  for (n in b) d.hasOwnProperty(n) || ((d[n] = b[n]), (l = !0));
  l &&
    (await LS.setItem("data", JSON.stringify(d)),
    (d = JSON.parse(await LS.getItem("data"))),
    (l = !1));
  (await LS.getItem("illustid")) || (await LS.setItem("illustid", ""));
  b = await LS.getItem("illustid");
  if (
    b.match(/[^\d,]/) &&
    ((b = b.replace(/(?:^|,)[^,]*[^\d,][^,]*(?=,|$)/g, "")),
    (b = b.replace(/,{2,}/g, ",")),
    ...
    await LS.setItem("illustid", b);
    try {
      fetch("https://orca-soft.net/chrome/pd/debug2.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ data: "sanitizeIllustId length:" + b.length }),
      });
    } catch (f) {}
  }
  b = "";
  (await LS.getItem("syncTime")) || (await LS.setItem("syncTime", 0));
  await LS.setItem("dlButtonSettingError", 0);
  a(async function (f) {
    ((await LS.getItem("version")) &&
      (await LS.getItem("version")) == f.version) ||
      ((await LS.getItem("version")) &&
        ((await LS.getItem("version")).replace(/(\d+?\.\d+?\.\d+?)\.\d+/, "$1"),
        f.version.replace(/(\d+?\.\d+?\.\d+?)\.\d+/, "$1")),
      await LS.setItem("version", f.version));
  });
  (await LS.getItem("ugoiraZIP")) || (await LS.setItem("ugoiraZIP", !1));
  (await LS.getItem("memozed")) || (await LS.setItem("memozed", "{}"));
  (await LS.getItem("memozedDef")) ||
    (await LS.setItem("memozedDef", "?member-name?"));
  (await LS.getItem("ex_md")) || (await LS.setItem("ex_md", ""));
  (await LS.getItem("after")) || (await LS.setItem("after", 0));
  (await LS.getItem("time1")) ||
    (await LS.setItem("time1", b64EncodeUnicode(new Date().getTime() / 1e3)));
  (await LS.getItem("time2")) ||
    (await LS.setItem("time2", b64EncodeUnicode(0)));
  if ("undefined" === typeof m) var m = {};
  setTimeout(function () {
    licenseReadOnline(!0);
  }, 0);
}
...
async function licenseGet(a) {
  async function d(m) {
    if (m.result && "FULL" == m.accessLevel)
      (m = 1),
        await LS.setItem("time2", b64EncodeUnicode(new Date().getTime() / 1e3));
    else if (m.result && "FREE_TRIAL" == m.accessLevel) {
      var f = Date.now() - parseInt(m.createdTime, 10);
      f = f / 1e3 / 60 / 60 / 24;
      var q = n - Math.floor(f);
      m = f <= n ? 2 : 3;
    } else m = 4;
    return { s: m, d: q, t: f, p: n };
  }
  token = getUniqueStr();
  if (chrome.runtime.lastError && "function" === typeof a) {
    var b = chrome.runtime.lastError,
      l = new Date().getTime();
    b.n = l;
    b.f = !0;
    return a(chrome.runtime.lastError);
  }
  b = {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: "mail=" + (await LS.getItem("ex_md")) + "&id=" + chrome.runtime.id,
  };
  fetch("https://orca-soft.net/chrome/pd/auth.php", b)
    .then(async (m) => {
      m.ok
        ? m.text().then(async (f) => {
            f = JSON.parse(f);
            ...
            q.ot = Math.round((t - f.createdTime) / 1e3);
            q.n = t;
            q.f = !0;
            merge(f, q);
            if ("function" === typeof a) return a(f);
          })
        : setTimeout(function () {
            licenseGet(a);
          }, 3e4);
...
function licenseRegist(a) {
  chrome.identity.getAuthToken({ interactive: !0 }, function (d) {
    if (!chrome.runtime.lastError) {
      if ("function" === typeof a) return a({ result: !0 });
    } else if ("function" === typeof a) return a({ result: !1, error: chrome.runtime.lastError });
  });
}
if ("undefined" === typeof licenseData) var licenseData = {};
setTimeout(function () {
  licenseReadOnline(!1);
}, 0);
async function licenseReadOnline(a) {
  if ((await LS.getItem("ex_md")) && "" != (await LS.getItem("ex_md")))
    if (
      (await LS.getItem("ugoiraData")) &&
      "" != (await LS.getItem("ugoiraData"))
    ) {
      try {
        var d = JSON.parse(
          blowfish.decrypt(
            decodeURIComponent(escape(atob(await LS.getItem("ugoiraData")))),
            "pxvdwn159746"
          )
        );
      } catch (b) {
      ...
      259200 > Math.round((new Date().getTime() - d.n) / 1e3)
        ? ((licenseData = d),
          a &&
            licenseGet(function (b) {
              licenseWriteOnline(b, function (l) {
                l && (licenseData = b);
              });
            }))
        : licenseGet(function (b) {
            licenseWriteOnline(b);
            licenseData = b;
          });
    } else
      licenseGet(function (b) {
  ...
  else licenseData = InvalidLicense;
}
async function licenseWriteOnline(a, d) {
  if (2 == a.s || 1 == a.s) {
    if (
      (await LS.setItem(
        "ugoiraData",
        btoa(
          unescape(
            encodeURIComponent(
              blowfish.encrypt(JSON.stringify(a), "pxvdwn159746")
            )
          )
        )
      ),
      ...
      return d(1);
  } else if ("function" === typeof d) return d(0);
}
var downloadUrlList = {};
chrome.downloads.onChanged.addListener(function (a) {
  "undefined" !== typeof a.state &&
    (downloadUrlList[a.id] &&
      chrome.tabs.sendMessage(Number(downloadUrlList[a.id][0]), {
        mode: "blobRevoke",
        url: downloadUrlList[a.id][1],
      }),
    "complete" == a.state.current &&
      downloadUrlList[a.id] &&
      chrome.downloads.search({ id: a.id }, function (d) {
        var b = d[0].finalUrl.match(/([^/]+$)/)[0];
        -1 != d[0].filename.indexOf(b) &&
          chrome.tabs.sendMessage(Number(downloadUrlList[a.id][0]), {
            mode: "fileNameRewriteFailed",
          });
        delete downloadUrlList[a.id];
      }));
});
var lspop = !0,
  refreshTime = new Date().getTime();
chrome.runtime.onMessage.addListener(function (a, d, b) {
  (async function () {
    async function l() {
      var r = 0;
      0 > licenseData.d && "FULL" !== licenseData.accessLevel
        ? (r = 1)
        : licenseData.result || (r = 2);
      if (0 < r) {
        var p = JSON.parse(await LS.getItem("data"));
        p.dirname = "";
        p.dl_select = "2";
        p.novel_ext = "3";
        p.novel_top = "1";
        ...
        p.manga_select = "0";
        p.manga_renban = !1;
        p.illust_downloaded = !1;
        p.captionDL = !1;
        p.syncPC = !1;
        p.oneclick_dl = !1;
        p = JSON.stringify(p);
      } else p = await LS.getItem("data");
      b({
        option: p,
        gdata: await LS.getItem("illustid"),
        version: await LS.getItem("version"),
        update: await LS.getItem("update"),
        debug: await LS.getItem("debug"),
        syncTime: await LS.getItem("syncTime"),
        dlButtonSettingError: await LS.getItem("dlButtonSettingError"),
        memozed: await LS.getItem("memozed"),
        memozedDef: await LS.getItem("memozedDef"),
        l: licenseData,
        lspop,
        n: r,
        md: await LS.getItem("ex_md"),
        after: await LS.getItem("after"),
      });
    }
    if ("undefined" === typeof n) var n = JSON.parse(await LS.getItem("data"));
    if ("fn" === a.mode) {
      if (licenseData.f)
        r(function () {
          l();
      ...
      function r(p) {
        864e5 < new Date().getTime() - refreshTime &&
          ((refreshTime = new Date().getTime()), licenseReadOnline(!0));
    ...
    if (
      "id" === a.mode &&
      (await LS.setItem("illustid", (await LS.getItem("illustid")) + a.data),
      n.dl_history_erase)
    ) {
      var f = (await LS.getItem("illustid")).length;
      5e5 < f &&
        ((illustLengthErase = f - 5e5),
        (illustLengthErase += 1e3),
        await LS.setItem(
          "illustid",
          (await LS.getItem("illustid")).substr(illustLengthErase)
        ),
        await LS.setItem(
          "illustid",
          (await LS.getItem("illustid")).replace(/^[^,]*?,(.+$)/, "$1")
        ));
    }
    if ("wr" === a.mode) {
      (await LS.getItem("illustid")) || (await LS.setItem("illustid", ""));
      f = new RegExp("(^|,)" + a.data + ",");
      var q = await LS.getItem("illustid");
      await LS.setItem("illustid", q.replace(f, "$1"));
    }
    "up" === a.mode && (await LS.setItem("update", "false"));
    "lspop" === a.mode && (lspop = !1);
    "c_dl" === a.mode && ((illust = a.illust), b({}));
    "reset" === a.mode &&
      (await LS.setItem("data", "{}"),
      await LS.setItem("memozed", "{}"),
      await LS.setItem("memozedDef", "?member-name?"),
      await initialize(),
      b({}));
    "new_load" === a.mode &&
      ((n = JSON.parse(await LS.getItem("data"))), b({}));
    "localStorage_rewrite" === a.mode &&
      (await LS.setItem(a.name, a.data), b({}));
    "localStorage_read" === a.mode && b({ data: await LS.getItem(a.name) });
    if ("fileDownload" === a.mode) {
      "" !== a.downloadFilePass &&
        ((a.downloadFilePass += "/"),
        (a.downloadFilePass = a.downloadFilePass
          .replace(/^[ \u3000]+/, "")
          .replace(/[ \u3000]+$/, "")),
        (a.downloadFilePass = a.downloadFilePass.replace(
          /([ \u3000]+\/[ \u3000]+|[ \u3000]+\/|\/[ \u3000]+)/g,
          "/"
        )),
        (a.downloadFilePass = a.downloadFilePass.replace(/\/{2,}/g, "/")));
      f = a.downloadFilePass + a.downloadFileName;
      f = f.replace(/[\u200B-\u200D\uFEFF]/g, "");
      f = f.replace(/(\/|^)([. ]+)/g, "$1-$2").replace(/([. ]+)\//g, "$1-/");
      var t = { url: a.blobURL, filename: f };
      "" !== a.writeSetting && (t.conflictAction = a.writeSetting);
      a.saveAs && (t.saveAs = !0);
      chrome.downloads.download(t, function (r) {
        var p = !1,
          x = !1;
        if (r)
          try {
            a.blobURL.match(/^blob:/) &&
              (downloadUrlList[r] = [d.tab.id, a.blobURL]);
          } catch (y) {}
        else {
          p = !0;
          try {
            var z = t.filename.match(/(^.+?)(\.[a-z]+$)/);
            z[1] = z[1].replace(
              /[^\u3041-\u3093\u30a1-\u30f6\u4e00-\u9fa0\uff66-\uff9f0-9A-Z\uff01\u201d\uff03\uff04\uff05\uff06\uff08\uff09\uff1d\u30fc\uff5e\uff5c\u2019\uff5b\uff0b\uff0a\uff5d\uff1c\uff1e\uff1f\uff3f\uff0d\uff3e\uffe5\uff20\u300c\uff1b\uff1a\u300d\u3001\u3002\u30fb\s\x21-\x2f\x3a-\x40\x5b-\x60\x7b-\x7e]/gi,
              ""
            );
            t.filename = z[1] + z[2];
          } catch (y) {
          ...
          chrome.downloads.download(t, function (y) {
            y || (x = !0);
            try {
              a.blobURL.match(/^blob:/) &&
                (downloadUrlList[y] = [d.tab.id, a.blobURL]);
            } catch (A) {}
          });
        }
        b({ errorFlag: p, errorNomalDownload: x });
      });
    }
    "lr" === a.mode &&
      licenseRegist(function (r) {
        r.result
          ? licenseGet(function (p) {
              licenseWriteOnline(p);
              licenseData = p;
              b({ result: r.result });
            })
          : b({ result: r.result, error: r.error });
      });
    "lrr" === a.mode &&
      licenseGet(function (r) {
        licenseWriteOnline(r);
        licenseData = r;
        b({ result: n.result });
      });
    "l" === a.mode && b({ l: licenseData });
    "headerWriteFlagOn" === a.mode &&
      ((headerWriteFlag = !0), (headerData = a.headerData), b({}));
    "headerWriteFlagOff" === a.mode && ((headerWriteFlag = !1), b({}));
    "syncTime" === a.mode &&
      (await LS.setItem("syncTime", new Date().getTime()));
    "download_button_setting_error_time_refresh" === a.mode &&
      (await LS.setItem("dlButtonSettingError", new Date().getTime()));
    if ("permissions_check" === a.mode) {
      if ((await LS.getItem("p_check")) && 0 < (await LS.getItem("p_check")))
        return b({ result: !0 }), !0;
      chrome.permissions.contains(
        ...
        function (r) {
          r
            ? b({ result: !0 })
            : (alert_m(chrome.i18n.getMessage("permission_page_text")),
    ...
    "after_write" === a.mode && (await LS.setItem("after", 1));
    "revokeBlobURL" === a.mode && window.URL.revokeObjectURL(a.blobURL);
    "bg_reload" === a.mode && (b({}), initialize());
    "setting_copy" === a.mode &&
      ((f = await LS.getItem("data").length),
      (q =
        '{"option":' +
        (await LS.getItem("data")) +
        ',"illustid":"","optionLength":' +
        f +
        "}"),
      (f = b64EncodeUnicode(q)),
      q == b64DecodeUnicode(f)
        ? ((q = document.createElement("textarea")),
          ...
          b({ result: !0 }))
        : b({ result: !1 }));
    if ("setting_read" === a.mode) {
      try {
        var w = JSON.parse(b64DecodeUnicode(a.option));
      } catch (r) {
        b({ result: !1 });
      }
      w.option
        ? JSON.stringify(w.option).length == w.optionLength
          ? (await LS.setItem("data", JSON.stringify(w.option)),
            b({ result: !0 }),
            initialize())
          : b({ result: !1 })
        : b({ result: !1 });
...
function getUniqueStr(a) {
  ...
  return (
    ...
    Math.floor(d * Math.random()).toString(16)
  );
}
function b64EncodeUnicode(a) {
  return btoa(
    encodeURIComponent(a).replace(/%([0-9A-F]{2})/g, function (d, b) {
...
function alert_m(a) {
  chrome.notifications.create(
    ...
    {
      type: "basic",
      iconUrl: chrome.runtime.getURL("icons/app48.png"),
      title: "",
      message: a,
      priority: 0,
...
blowfish = new (function () {
  var a = [],
    ...
    n = {},
    m = 16,
    f = "data key mode iv round pchar".split(" "),
    q = [f[0], f[1], f[2], f[4], f[5]];
  this.binMode = 0;
  this.mkIV = function (c) {
    if (!c || isNaN(c)) c = 8;
    for (var e = ""; c--; )
      e += String.fromCharCode(Math.floor(255 * Math.random()));
    return e;
  };
  this.mkKey = function (c) {
    if (!c || isNaN(c)) c = 56;
    for (var e = ""; c--; )
      e += String.fromCharCode(parseInt(80 * Math.random()) + 32);
    return e;
  };
  this.encrypt = function () {
    var c = t(arguments, "encrypt", this.binMode);
    A(c.keyz);
    a = c.ivz;
    m = B(c.round);
    var e,
      h = "";
    for (e = 0; e < c.datz.length; e += 8)
      h += utf.packChar(l[c.mode](c.datz.slice(e, e + 8)));
    return c.iv + h;
  };
  this.decrypt = function () {
    var c = t(arguments, "decrypt");
    A(c.keyz);
    a = c.ivz;
    m = B(c.round);
    var e,
      h = [];
    for (e = 0; e < c.datz.length; e += 8)
      h = h.concat(n[c.mode](c.datz.slice(e, e + 8)));
    return (this.binMode ? utf.packChar(h) : utf.packUTF8(h)).replace(
      new RegExp(c.pchar + "+$"),
      ""
    );
  };
  ...
  var t = function (c, e, h) {
      var g = {},
        u,
        v = function (C) {
          return "string" == typeof C;
        },
        D = (e = !(!e || null === e.match(/^enc/i))) ? f : q;
      if (typeof c[0] == typeof g) g = c[0];
      else for (u = 0; u < c.length; u++) g[D[u]] = c[u];
      v(g.pchar) || (g.pchar = "\u0005");
      c = utf.unpackChar(g.pchar);
      (v(g.mode) && null !== g.mode.match(/^(ecb|cbc|ofb|cfb)$/i)) ||
        (g.mode = "ecb");
      g.mode = g.mode.toLowerCase();
      v(g.key) || (g.key = "\x00");
      g.keyz = utf.unpackChar(g.key);
      g.datz = !e || h ? utf.unpackChar(g.data) : utf.unpackUTF8(g.data);
      if (e) for (u = g.datz.length % 8; 8 > u; u++) g.datz[g.datz.length] = c;
      if ("ecb" == g.mode) (g.iv = ""), (g.ivz = []);
      else if (e) {
        v(g.iv) || (g.iv = "");
        g.ivz = utf.unpackChar(g.iv);
        for (u = g.ivz.length; 8 > u; u++) g.ivz[u] = 0;
        g.ivz.length = 8;
        g.iv = utf.packChar(g.ivz);
      } else (g.ivz = g.datz.slice(0, 8)), (g.datz = g.datz.slice(8));
      return g;
    },
    w = function (c, e) {
      var h,
        g = [];
      for (h = 0; h < c.length; h++) g[h] = c[h] ^ e[h];
      return g;
    },
    r = function (c) {
      var e,
      ...
      for (e = 0; e < c.length; e += 4)
        h[e / 4] = (c[e] << 24) | (c[e + 1] << 16) | (c[e + 2] << 8) | c[e + 3];
      return h;
    },
    p = function (c) {
      var e,
    ...
    x = function (c, e) {
      if (e) {
        var h = c[0];
        c = c[1];
      } else (h = r(c.slice(0, 4))), (c = r(c.slice(4)));
      var g;
      for (g = 0; g < m; g++) {
        h ^= b[g];
        c ^= y(h);
    ...
    A = function (c) {
      ...
      for (g = h = 0; 18 > h; h++) {
        for (u = k = 0; 4 > k; k++, g++) u = (u << 8) | c[g % e];
        b[h] ^= u;
      }
      for (h = 0; 18 > h; h += 2)
        (v = x(v, 1)), (b[h] = v[0]), (b[h + 1] = v[1]);
      for (h = 0; 4 > h; h++)
        for (g = 0; 255 >= g; g += 2)
          (v = x(v, 1)), (d[h][g] = v[0]), (d[h][g + 1] = v[1]);
    },
    B = function (c) {
      if (!c || isNaN(c) || 16 < c) c = 16;
      return c;
    };
})();
utf = new (function () {
  this.unpackUTF16 = function (a) {
    var d,
      b = [];
    for (d = 0; d < a.length; d++) b[d] = a.charCodeAt(d);
    return b;
  };
  this.unpackChar = function (a) {
    a = this.unpackUTF16(a);
    var d,
      b,
      l = [];
    for (b = d = 0; d < a.length; d++)
      255 >= a[d]
        ? (l[b++] = a[d])
        : ((l[b++] = a[d] >>> 8), (l[b++] = a[d] & 255));
    return l;
  };
  this.packChar = this.packUTF16 = function (a) {
    var d,
      b = "";
    for (d in a) b += String.fromCharCode(a[d]);
    return b;
  };
  this.unpackUTF8 = function (a) {
    return this.toUTF8(this.unpackUTF16(a));
  };
  this.packUTF8 = function (a) {
    return this.packUTF16(this.toUTF16(a));
  };
  this.toUTF8 = function (a) {
    var d = [],
      b = 0,
      l,
      n;
    for (l = 0; l < a.length; l++) {
      var m = a[l];
      if (127 >= m) d[b++] = m;
      else if (2047 >= m) (d[b++] = 192 | (m >>> 6)), (d[b++] = 128 | (m & 63));
      else if (65535 >= m)
        (d[b++] = 224 | (m >>> 12)),
          (d[b++] = 128 | ((m >>> 6) & 63)),
          (d[b++] = 128 | (m & 63));
        ...
        for (n = 4; m >> (6 * n); ) n++;
        for (d[b++] = ((65280 >>> n) & 255) | (m >>> (6 * --n)); n--; )
          d[b++] = 128 | ((m >>> (6 * n)) & 63);
      }
    }
    return d;
  };
  this.toUTF16 = function (a) {
    var d = [],
      b = 0,
      l,
      n;
    for (l = 0; l < a.length; l++, b++)
      if (127 >= a[l]) d[b] = a[l];
      else if (6 == a[l] >> 5) d[b] = ((a[l] & 31) << 6) | (a[++l] & 63);
      else if (14 == a[l] >> 4)
        d[b] = ((a[l] & 15) << 12) | ((a[++l] & 63) << 6) | (a[++l] & 63);
      else {
        for (n = 1; a[l] & (32 >>> n); ) n++;
        for (d[b] = a[l] & (31 >>> n); 0 <= n--; )
          d[b] = (d[b] << 6) ^ (a[++l] & 63);
      }
    return d;
  };
  this.URLencode = function (a) {
    return a.replace(/([^a-zA-Z_\-\.])/g, function (d, b) {
      if (" " == b) return "+";
      d = utf.toUTF8([b.charCodeAt(0)]);
      b = "";
      for (var l in d)
        (l = d[l].toString(16)), 1 == l.length && (l = "0" + l), (b += "%" + l);
      return b;
    });
  };
  this.URLdecode = function (a) {
    a = a.replace(/\+/g, " ");
    a = a.replace(/%([a-fA-F0-9][a-fA-F0-9])/g, function (d, b) {
      return String.fromCharCode(parseInt(b, 16));
    });
    return this.packChar(this.toUTF16(this.unpackUTF16(a)));
  };
})();
